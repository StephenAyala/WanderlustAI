...
const debounce = (
  ...
  let lastArgs: any[] | undefined; // Allow undefined
  let lastThis: any | undefined; // Allow undefined
  ...
  if (isObject(options)) {
    ...
    maxWait = maxing
      ? Math.max(options.maxWait !== undefined ? +options.maxWait : 0, wait)
      : maxWait;
  ...
  const startTimer = (pendingFunc: () => void, milliseconds: number) => {
    if (useRAF) {
  ...
  const cancelTimer = (id: ReturnType<typeof setTimeout> | number) => {
    if (useRAF) {
